...
class Cutout(Augmentation):
    def __init__(self, batch_size: int, do_prob: float, sequence_shape: Union[list, tuple], min_cutout_len: int,
                 max_cutout_len: int, channel_drop_prob: float):
        """
    ...
    def get_length_wise_cut_array(self):
        ...
        do = tf.cast(tf.random.uniform((), ) < self.do_prob, tf.float32)
    ...
    def get_cutout_mask(self, nothing:tf.Tensor):
        '''
